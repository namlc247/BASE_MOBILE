...
export const fetchUserDetail = async (id: number, userName: string): Promise<UserDetailResponse> => {
  try {
    const response = await instance.post<UserDetailResponse>('/gateway/user/detail', {
      id,
      userName,
    });
